...
def test_fahrenheit_kelvin():
    ...
    assert fahrenheit_kelvin(32) == approx(273.15, rel=1e-4)  # Ponto de congelamento da água
    assert fahrenheit_kelvin(212) == approx(373.15, rel=1e-4)  # Ponto de ebulição da água
    assert fahrenheit_kelvin(-40) == approx(233.15, rel=1e-4)  # Teste com valores negativos
    assert fahrenheit_kelvin(98.6) == approx(310.15, rel=1e-4)  # Temperatura corporal média

def test_kelvin_fahrenheit():
    """Testa a conversão de Kelvin para Fahrenheit"""
    assert kelvin_fahrenheit(273.15) == approx(32, rel=1e-4)  # Ponto de congelamento da água
    assert kelvin_fahrenheit(373.15) == approx(212, rel=1e-4)  # Ponto de ebulição da água
    assert kelvin_fahrenheit(0) == approx(-459.67, rel=1e-4)  # Zero absoluto
    assert kelvin_fahrenheit(310.15) == approx(98.6, rel=1e-4)  # Temperatura corporal média
...
def test_milhas_quilometros():
    ...
    assert milhas_quilometros(1) == approx(1.60934, rel=1e-4)  # 1 milha = 1.60934 quilômetros
    assert milhas_quilometros(5) == approx(8.0467, rel=1e-4)  # 5 milhas = 8.0467 quilômetros
    assert milhas_quilometros(0) == 0  # 0 milhas = 0 quilômetros
    assert milhas_quilometros(0.5) == approx(0.80467, rel=1e-4)  # Valor não inteiro

def test_quilometros_milhas():
    """Testa a conversão de quilômetros para milhas"""
    assert quilometros_milhas(1) == approx(0.621371, rel=1e-4)  # 1 quilômetro = 0.621371 milhas
    assert quilometros_milhas(5) == approx(3.106855, rel=1e-4)  # 5 quilômetros = 3.106855 milhas
...
def test_quilos_libras():
    ...
    assert quilos_libras(1) == approx(2.20462, rel=1e-4)  # 1 quilograma = 2.20462 libras
    assert quilos_libras(5) == approx(11.0231, rel=1e-4)  # 5 quilogramas = 11.0231 libras
    assert quilos_libras(0) == 0  # 0 quilograma = 0 libras
    assert quilos_libras(10.5) == approx(23.1495, rel=1e-4)  # Valor não inteiro

def test_libras_quilos():
    """Testa a conversão de libras para quilogramas"""
    assert libras_quilos(2.20462) == approx(1, rel=1e-4)  # 2.20462 libras = 1 quilograma
    assert libras_quilos(11.0231) == approx(5, rel=1e-4)  # 11.0231 libras = 5 quilogramas
    assert libras_quilos(0) == 0  # 0 libras = 0 quilogramas
    assert libras_quilos(23.1495) == approx(10.5, rel=1e-4)  # Valor não inteiro
...
def test_gramas_quilos():
    ...
    assert gramas_quilos(234) == approx(0.234, rel=1e-4)  # Valor não inteiro
...
def test_mps_mph():
    ...
    assert mps_mph(1) == approx(2.23694, rel=1e-4)  # 1 metro por segundo = 2.23694 milhas por hora
    assert mps_mph(5) == approx(11.1847, rel=1e-4)  # 5 metros por segundo = 11.1847 milhas por hora
    assert mps_mph(0) == 0  # 0 metros por segundo = 0 milhas por hora
    assert mps_mph(10.5) == approx(23.47437, rel=1e-4)  # Valor não inteiro
...
def test_mph_mps():
    ...
    assert mph_mps(23.47437) == approx(10.5, rel=1e-4)  # Valor não inteiro

def test_kph_mps():
    """Testa a conversão de quilômetros por hora para metros por segundo"""
    assert kph_mps(1) == approx(0.277778, rel=1e-4)  # 1 quilômetro por hora = 0.277778 metros por segundo
    assert kph_mps(5) == approx(1.38889, rel=1e-4)  # 5 quilômetros por hora = 1.38889 metros por segundo
    assert kph_mps(0) == 0  # 0 quilômetros por hora = 0 metros por segundo
    assert kph_mps(10.5) == approx(2.91667, rel=1e-4)  # Valor não inteiro
...
def test_mps_kph():
    ...
    assert mps_kph(10.5) == approx(37.8, rel=1e-4)  # Valor não inteiro

def test_kph_mph():
    """Testa a conversão de quilômetros por hora para milhas por hora"""
    assert kph_mph(1) == approx(0.621371, rel=1e-4)  # 1 quilômetro por hora = 0.621371 milhas por hora
    assert kph_mph(5) == approx(3.106855, rel=1e-4)  # 5 quilômetros por hora = 3.106855 milhas por hora
    assert kph_mph(0) == 0  # 0 quilômetros por hora = 0 milhas por hora
    assert kph_mph(10.5) == approx(6.5244, rel=1e-4)  # Valor não inteiro

def test_mph_kph():
    """Testa a conversão de milhas por hora para quilômetros por hora"""
    assert mph_kph(1) == approx(1.60934, rel=1e-4)  # 1 milha por hora = 1.60934 quilômetros por hora
    assert mph_kph(5) == approx(8.0467, rel=1e-4)  # 5 milhas por hora = 8.0467 quilômetros por hora
    assert mph_kph(0) == 0  # 0 milhas por hora = 0 quilômetros por hora
    assert mph_kph(10.5) == approx(16.8989, rel=1e-4)  # Valor não inteiro
